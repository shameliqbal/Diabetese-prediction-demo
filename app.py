...
def inject_css():
    ...
    /* FIX: readable labels on Streamlit Cloud */
label, [data-testid="stWidgetLabel"], [data-testid="stWidgetLabel"] p, .stSlider label p { color: #0f172a !important; font-weight: 800 !important; font-size: 0.98rem !important; }
[data-testid="stMetricLabel"], [data-testid="stMetricLabel"] p, [data-testid="stMetricValue"], [data-testid="stMetricValue"] div { color: #0f172a !important; }
.stSelectbox div[data-baseweb="select"] > div { background: #ffffff !important; color: #0f172a !important; border: 1px solid #cbd5e1 !important; border-radius: 12px !important; }
.stSelectbox div[data-baseweb="select"] span { color: #0f172a !important; }
[data-baseweb="tab"] p { color: #0f172a !important; font-weight: 850 !important; }
</style>
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["1. Patient profile", "2. Medications", "3. Labs & history"])
    ...
    with tab1:
        ...
        with c1:
            ...
            race = st.selectbox("Race / ethnicity group", RACE_OPTIONS, help="Unknown values are treated as UNK.")
            gender = st.selectbox("Gender", GENDER_OPT)
            age = st.selectbox("Age group / patient age band", AGE_OPTIONS, index=5)
        with c2:
            st.markdown("#### Encounter details")
            admission_type_label = st.selectbox("Admission type: emergency, urgent, elective, etc.", list(ADMISSION_TYPE.keys()))
            admission_src_label = st.selectbox("Admission source: where the patient came from", list(ADMISSION_SRC.keys()), index=6)
            discharge_label = st.selectbox("Discharge disposition: where the patient went after care", list(DISCHARGE_DISP.keys()))
        with c3:
            st.markdown("#### Care context")
            med_spec = st.selectbox("Treating medical specialty", MED_SPEC_OPTIONS)
            payer_code = st.selectbox("Insurance / payer code", PAYER_OPTIONS, index=16)
            has_weight = st.checkbox("Weight value was recorded", value=False, help="Only whether weight exists is used.")
        time_in_hospital = st.slider("Time in hospital / length of stay", 1, 14, 4)
    ...
    with tab2:
        ...
        change = s1.selectbox("Any diabetes medication changed during encounter?", CHG_OPTIONS, help="Ch means one or more diabetes medications changed.")
    ...
    with tab3:
        ...
        with c1:
            ...
            max_glu_serum = st.selectbox("Max glucose serum test result", GLU_OPTIONS)
            A1Cresult = st.selectbox("A1C test result", A1C_OPTIONS)
        with c2:
